...
public class EventManager : MonoBehaviour
{
    ...
    private List<EventWeight> calculatedEventWeights = new List<EventWeight>();
    ...
    private void FixedUpdate() {
        ...
        if(throwTimer <= 0) ThrowingTick();
    }

    public void ThrowingTick() {
    ...
    private void Start() { CalculateWeights(); }
    public void CacheEvents(GameObject[] objects)
    {
        eventSlots.Clear();
        foreach (var eventPiece in objects) {
            eventSlots.Add(eventPiece.GetComponent<IEvent>());
        }
    }
